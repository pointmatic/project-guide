...
**Resolved in R.d — neither strip nor keep, but fall back at runtime.** The registration line is rewritten to `complete -o nosort -F … 2>/dev/null || complete -F …`, so bash ≥ 4.4 takes the first form and keeps Click's ordering while 3.2's `invalid option name` error is swallowed (silent degradation) and the fallback registers. Verified on bash 3.2.57 and 5.3.15: 13 completions each, binary off `PATH`. The transform is applied independently of `--bin` — unlike the Amendment 3 post-processing, this is a defect in Click's template rather than in how the callback resolves its binary. **Consequence for R.i:** the residual macOS 3.2 limitation is only `compopt` breaking dir/file completions; "nothing registers at all" is no longer true.
