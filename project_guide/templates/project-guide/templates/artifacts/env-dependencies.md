...
# env-dependencies.md -- {{project_name}} ({{programming_language}})

This document formally enumerates the **named environments** the `{{project_name}}` repo needs:
...
<!-- HOW TO FILL: Remove this entire section from the generated document. It is guidance only. -->
...
- The `backend`, `languages`, `frameworks`, `packaging`, and `app_type` vocabularies in §2 are **Pyve-owned and closed**. Never invent a value: a value outside the set is a spec violation; a missing mechanism is a Pyve change-request (§8).
...
<!-- HOW TO FILL: Keep the canonical rows. Add a row for any non-canonical mechanism
     this repo actually relies on (e.g. "homebrew", "asdf-only"), marked status=advisory,
...
- **Files that must NOT be committed:** `<.venv/, .pyve/envs/, .env, ...>`.
